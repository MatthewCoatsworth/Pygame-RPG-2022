...
class Player(pygame.sprite.Sprite):
    ...
    def movement(self):
        ...
        if keys[pygame.K_LEFT]:
            #moves evrything in the all sprites group (which includes the player) to the right
            for sprite in self.game.all_sprites:
                sprite.rect.x += PLAYER_SPEED
            #moves the player to the left so the player stays in the center of the screen
...
class NPC1(pygame.sprite.Sprite):
    ...
    def update(self):
    #if the player touchs the key make it dissapar from the map and set the variable for "haskey" to true
...
class Door(pygame.sprite.Sprite):
    ...
    def __init__(self, game, x, y):
        ...
        #match size of Hitbox to the same dimensions as the Sprite selected on the style sheet
        self.rect = self.image.get_rect()
        self.rect.x = self.x
        self.rect.y = self.y
...
class Lock(pygame.sprite.Sprite):
    ...
    def __init__(self, game, x, y):
        ...
        #the width and height is increased to insure the players hit box cannot come into contact with the hit box of the door
        self.x = x * TILESIZE - 5
        self.y = y * TILESIZE -5
    ...
    #removes the key the invisible barrier over the door from the game when the key is collected
    def update(self):
        if self.game.player.haskey == True:
            self.kill()
        if pygame.Rect.colliderect(self.rect, self.game.player.rect):
            self.game.displaytext = 'NO'
...
class Textbox(pygame.sprite.Sprite):
    ...
    def __init__(self, game, x, y):
        ...
        #add sprites to the textbox group. the other spirtes were added to the "game.all_sprites" group however as these sprites move when the arrow keys are pressed I added the textbox to a diffrent group
